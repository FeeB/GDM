...
public class GLDM_U1_s0538153 implements PlugIn {
	...
	public void run(String arg) {
		...
		if (choice.equals("Schwarzes Bild")) {
			...
			// Mittelpunkt & Radius bestimmen
			double mittelPunktX = width / 2;
			double mittelPunktY = height / 2;
			double radius = height / 3.2;
			// Wert fŸr Bestimmung der Punkte zum Mittelpunkt
			int distance = 0;
			// Schleife ueber die x-Werte
			for (double x = 0; x < width; x++) {
				// Schleife ueber die y-Werte
				for (double y = 0; y < height; y++) {
					// Formel fŸr Bestimmung der Punkte zum Mittelpunkt
					...
					if (distance < radius) {
						...
						// Werte zurueckschreiben
						pixels[pos] = 0xFF000000 | (r << 16) | (g << 8) | b;
					}// falls Distance grš§er Radius>>wei§ 
					else {
						int pos = (int) (y * width + x); // Arrayposition
															// bestimmen
						int r = 255;
			...
			for (double x = 0; x < width; x++) {
				...
				for (double y = 0; y < height; y++) {
					...
					distance = (int) (Math.sqrt(Math.pow(mittelPunktX - x, 2)
							+ Math.pow(mittelPunktY - y, 2)));
					// Unterschied zwischen Abstand zum Mittelpunkt und Radius,fŸr Bereich fŸr die weiche Kante
					double deltaDR = distance - radius;
					// Bereich fŸr den roten Kreis
					if (distance < radius) {
						int pos = (int) (y * width + x); // Arrayposition
															// bestimmen
						int r = 255;
						...
						// Bereich fŸr die weiche Kante
						if (distance >= radius && distance < radius2) {
							int pos = (int) (y * width + x); // Arrayposition  bestimmen
							
							//	der Šu§ere Radius2 ist 25 px grš§er als der Radius1(ohne weiche Kante),
							// bei 256 Stufen ergibts sich der Faktor von ca. 10,20, um einen homogenen Verlauf zu erzeugen
							int r = 255;
