...
public class GRDM_U2 implements PlugIn {
	...
	class CustomWindow extends ImageWindow implements ChangeListener {
		...
		void addPanel() {
			...
			jSliderSaettigung = makeTitledSilder("S둻tigung 1.2", 0, 50, 0);
		...
		// Farbtransformation wird hier einmal f웦 alle kommenden
		// Transformationen durchgef웘rt laut Formel
		...
		private void changeContrast(ImageProcessor ip) {
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					...
					// Schwellenwert um alle Werte 웑er dem Schwellenwert
					...
					// Abfrage ob Wert 웑er oder unter dem Schwellenwert liegt,
		...
		private void farbDrehung(ImageProcessor ip) {
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					...
					// der eingegebene Wert 웑er den Regler. Hier ist toRadians
		...
		private void changeSaettigung(ImageProcessor ip) {
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					...
					// Farbtransformation von RGB zu YCbCr
					double bigY = transformationToY(r, g, b);
					// Farbwerte m웧sen * den saettigungswert des Reglers
					// genommen werden
		...
		// Funktion um Pixel zu begrenzen: alle Werte 웑er 255 werden zu 255,
